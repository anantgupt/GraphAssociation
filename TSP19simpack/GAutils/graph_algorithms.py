...
#        print('Target{}: x={},y={},vx={},vy={}'.format(tno+1, target_current.x, target_current.y,target_current.vx,target_current.vy))
    ...
    if estalgo==0:
        ...
        garda_sel = ea.meth2(np.copy(beat), sensors, Nob, [2,2])
    elif estalgo==2:
...
#    rd_error = np.array(prfe.compute_rd_error(garda_sel, gardat, plt))
    ...
    print(time.time()-t)
    t=time.time()
    min_gsigs, glen,_ = grpr.get_minpaths(G2, sensors, mode)
    rtime_algo[mode]= time.time()-t
    print(time.time()-t)
    plt.figure(75)
    ...
    pr.plot_scene(plt, scene, sensors, 75, 'Graph pruning detects {} targets'.format(len(min_gsigs)))
    #%%
    t=time.time()
    ...
    for sif in graph_sigs:
        l_cost, g_cost = mle.est_pathllr(sif, sensors, len(sensors)-1, False)
...
#%% Compute error measures
    ospa_error[f,:] = np.array(prfe.compute_ospa(scene, gr1_centers, sensors, gardat))
