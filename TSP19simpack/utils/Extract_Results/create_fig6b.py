...
def cf6b(mode = 'Relax', width = 3.45, height = 2.6, font_size = 8):
    ...
    fig_handle1 = pl.load(open('fig_rob-swidth0.2/plot10.pickle','rb'))
    fig_handle2 = pl.load(open('fig_rob-swidth0.4/plot10.pickle','rb'))
    fig_handle3 = pl.load(open('fig_rob-swidth0.8/plot10.pickle','rb'))
    ...
    mkr = [',','+','x']
    for i in range(3):
        rng1 = fig_handle2.axes[0].lines[0].get_data()[0]
    ...
    #    ax.plot(rng, mse1[i], 'b-', label='RMSE SNR=-10 dB')
        ax[0].plot(rng1, mse1[i], 'r-', marker=mkr[i], label=r'$\rho$='+str(i)+', $L_w$=0.2m')
        ax[0].plot(rng1, mse2[i], 'b-', marker=mkr[i], label=r'$\rho$='+str(i)+', $L_w$=0.4m')
        ax[0].plot(rng1, mse3[i], 'g-', marker=mkr[i], label=r'$\rho$='+str(i)+', $L_w$=0.8m')
    ...
    ax[1].plot(rng2, cnt2, 'b-o', label=r'$L_w$=0.8m')
    ax[1].plot(rng2, cnt3, 'g-s', label=r'$L_w$=1.6m')
